"""Tiny JSON-backed config for the MusicOrganizer GUI.

Stores the chosen theme ("light"/"dark") and a short list of recently-scanned
folders.  On Windows the file lives at
``%LOCALAPPDATA%\\MusicOrganizer\\config.json``; elsewhere it falls back to
``~/.musicorganizer/config.json``.  Every function is defensive -- a corrupt or
...
def config_dir():
    ...
    ``%LOCALAPPDATA%\MusicOrganizer`` on Windows, ``~/.musicorganizer`` otherwise.
    """
...
def add_recent(path):
    """Push *path* (a folder) to the front of the recent list, de-duplicated."""
    if not path:
        return
    try:
